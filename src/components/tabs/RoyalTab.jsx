...
import { useTipperInfo, useRoyalPoints }  from '../../hooks/useContractData';
...
const NEXT_TIER_THRESHOLDS = [0.01, 0.05, 0.1, 0.5, 20];

export default function RoyalTab() {
  const { address, isConnected } = useAccount();
  const { totalTipped, tipCount, tier, exists, isLoading } = useTipperInfo(address);
  const { points }       = useRoyalPoints(address);
  const { claimPoints, isLoading: claiming, isSuccess: claimed } = useClaimPoints();

  if (!isConnected) {
    return (
      <div style={{
        display: 'flex', flexDirection: 'column', alignItems: 'center',
        justifyContent: 'center', gap: '1rem', minHeight: '60vh', textAlign: 'center',
      }}>
  ...
  const progress     = nextThresh ? Math.min((tippedETH / nextThresh) * 100, 100) : 100;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
      ...
      {/* Points card */}
      <div style={{
        background: '#0052ff', borderRadius: '1.5rem', padding: '1.5rem',
        ...
        <div style={{
          ...
          letterSpacing: '0.1em', color: 'rgba(255,255,255,0.5)', marginBottom: '0.5rem',
        }}>
          Your Royal Points
        </div>
        <div style={{
          fontSize: '3rem', fontWeight: 800, color: '#fbbf24',
          WebkitTextStroke: '1px white', letterSpacing: '-0.02em',
        ...
        <div style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.65)' }}>
          ...
          {exists ? `Total: ${tippedETH.toFixed(4)} ETH` : 'No tips yet — start tipping!'}
        </div>
        <div style={{
      ...
      <div style={{
        ...
        <div style={{
          fontSize: '0.65rem', fontWeight: 700, textTransform: 'uppercase',
          letterSpacing: '0.08em', color: '#8b95a8', marginBottom: '1rem',
        }}>
          Current Tier
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem' }}>
          <div style={{
            width: '44px', height: '44px', background: tierStyle.bg,
          ...
          <div>
            <div style={{ fontSize: '1rem', fontWeight: 700, color: tierStyle.color }}>
              {tierName}
            </div>
        ...
        {nextTierName ? (
          <>
            <div style={{
              fontSize: '0.75rem', color: '#8b95a8', marginBottom: '6px',
          ...
          <div style={{
            fontSize: '0.825rem', fontWeight: 700, color: '#0052ff',
      ...
      <div style={{
        ...
        borderRadius: '1.25rem', padding: '1rem 1.25rem', display: 'flex', gap: '0.75rem',
      }}>
      ...
      {points > 0 && (
        ...
        >
          {claimed ? '✅ Points Claimed!' : claiming
            ? '⏳ Claiming...'
            : `Claim ${points.toLocaleString()} Points On-Chain →`}
        </button>
      )}
    </div>
  );
